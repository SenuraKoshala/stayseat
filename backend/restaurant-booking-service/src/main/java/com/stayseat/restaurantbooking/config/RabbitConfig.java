 ...
 *
 * <p>
 * Only the exchange is declared here - the queues and bindings are owned by
 * the Notification Service (the consumer). Declaring the same durable topic
 * exchange on both sides is idempotent, so it is safe regardless of start
 * order.
 */
@Configuration
public class RabbitConfig {
    ...
    public static final String PAYMENT_PROCESSED_QUEUE = "restaurant.payment.processed";
     ...
     * so {@code LocalDate} values serialize as ISO-8601 strings that the
     * Notification Service (Jackson 3) can read back into its payload types.
