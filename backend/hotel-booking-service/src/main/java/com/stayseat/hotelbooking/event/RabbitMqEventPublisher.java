...
public class RabbitMqEventPublisher implements EventPublisher {
    ...
    /**
     * Maps event types to topic routing keys. {@code hotel.confirmed} is the key
     * the Notification Service binds its hotel queue to; created/cancelled use
     * sibling keys that currently have no consumer (silently dropped by the
     * topic exchange) but are ready for future subscribers.
     */
    private String routingKeyFor(String eventType) {
        return switch (eventType) {
            case "HotelBookingCreated"   -> "hotel.created";
            case "HotelBookingConfirmed" -> "hotel.confirmed";
            case "HotelBookingCancelled" -> "hotel.cancelled";
            default -> "hotel." + eventType.toLowerCase();
        };
    }
}
